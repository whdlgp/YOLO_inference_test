...
// Perform inference
std::vector<ObjectDetection::Output> YOLOv8Model::infer(ObjectDetection::Input& input)
{
...
cv::Mat YOLOv8Model::convert_to_Mat(const ObjectDetection::Input& input)
{
    // CV_8UC3, CV_8UC1 
...
cv::Mat YOLOv8Model::preprocess(const cv::Mat &image)
{
    cv::Mat blob;
...
// Perform inference
std::vector<cv::Mat> YOLOv8Model::infer(const cv::Mat &blob)
{
    net.setInput(blob);
...
std::vector<ObjectDetection::Output> YOLOv8Model::postprocess(const cv::Mat &frame, const std::vector<cv::Mat> &outs)
{
    ...
    // YOLOv8 output is [1, 84, 8400] where 84 = 4 (bbox) + 80 (class_conf)
