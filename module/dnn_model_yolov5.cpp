...
void YOLOv5Model::init(const InitParams &model_files
...
cv::Mat YOLOv5Model::convert_to_Mat(const ObjectDetection::Input& input)
{
    // CV_8UC3, CV_8UC1 
...
cv::Mat YOLOv5Model::preprocess(const cv::Mat &image)
{
    cv::Mat blob;
...
// Perform inference
std::vector<cv::Mat> YOLOv5Model::infer(const cv::Mat &blob)
{
    net.setInput(blob);
...
std::vector<ObjectDetection::Output> YOLOv5Model::postprocess(const cv::Mat &frame, const std::vector<cv::Mat> &outs)
{
