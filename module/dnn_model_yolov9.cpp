#include "dnn_model_yolov9.hpp"

#include <fstream>
#include <opencv2/opencv.hpp>

void YOLOv9Model::init(const InitParams &model_files
...
// Perform inference
std::vector<ObjectDetection::Output> YOLOv9Model::infer(ObjectDetection::Input& input)
{
...
cv::Mat YOLOv9Model::convert_to_Mat(const ObjectDetection::Input& input)
{
    // CV_8UC3, CV_8UC1 
...
cv::Mat YOLOv9Model::preprocess(const cv::Mat &image)
{
    cv::Mat blob;
...
// Perform inference
std::vector<cv::Mat> YOLOv9Model::infer(const cv::Mat &blob)
{
    net.setInput(blob);
...
std::vector<ObjectDetection::Output> YOLOv9Model::postprocess(const cv::Mat &frame, const std::vector<cv::Mat> &outs)
{
    ...
    // YOLOv9 output is [1, 84, 8400] where 84 = 4 (bbox) + 80 (class_conf)
